...
(async () => {
  ...
  await page.addStyleTag({
    content: `
      ...
      * {
      ...
      /* Hero de cidade (.cidade-clean): o gradiente rgba do ::after fica empilhado sobre o
         background-image do ::before, o que o Ghostscript nao preserva corretamente (raster
         some na conversao de cor). Suprime os pseudo-elementos aqui; a foto de fundo e o
         gradiente sao recriados via <img class="cidade-hero-photo"> + tratamento no JS abaixo. */
      .cidade-clean::before, .cidade-clean::after { display: none !important; content: none !important; }

      /* Mesmo problema em #cidade .pais-hero: .pais-hero-overlay é uma div com classe (não
         inline) com gradiente rgba sobre o <img inset:0>. Suprimida aqui; a foto é restaurada
         via URL direta no JS abaixo (ver nota sobre border-radius logo abaixo). */
      .pais-hero-overlay { display: none !important; }

      /* border-radius + overflow:hidden num background-image pequeno (nao full-page) faz o
         Ghostscript descartar o raster (mesmo com URL http direta OU canvas recompactado —
         testado os dois). Unico fix que funcionou: remover o border-radius so no PDF. */
      ...
      /* Nota: removido o override "v1 compat" que forcava #menu-dias/#legendas .vg-card para azul
         solido — nesta versao (Vanessa) o .vg-card e projetado como card branco com texto escuro
         (#475569); forcar fundo azul deixava o texto ilegivel. Mantido apenas o fundo branco do
         .vg-card-header original (ja e azul por CSS proprio do documento, nao precisa de override). */

      /* Legenda glass: branco sólido (v1 compat) */
      .legenda-glass {
        background: white !important;
  ...
  await page.evaluate(async () => {
    ...
    // Capa: imagem + gradiente escuro de baixo
    await compositeSection('#capa', (ctx, W, H) => {
      const grad = ctx.createLinearGradient(0, 0, 0, H);
      grad.addColorStop(0,    'rgba(0,0,0,0.04)');
      grad.addColorStop(0.28, 'rgba(0,0,0,0)');
      grad.addColorStop(0.60, 'rgba(0,0,0,0.28)');
    ...
    // Heros de cidade (.cidade-clean): tratados fora deste evaluate (ver bloco cidadeHeroFiles
    // logo abaixo) — precisam gravar um PNG em disco e servir via URL http, nao data URI.
  ...
  // Correcao pontual: elementos "glass" (fundo branco translucido rgba(255,255,255,x) + texto
  // branco) sobre foto escura. O loop generico de conversao rgba->opaco acima faz blend com
  // branco — como o proprio rgba ja e branco, o resultado e branco solido, apagando o texto
  // branco por cima. Aplicado DEPOIS do evaluate (stylesheet !important vence o inline setado
  // pelo JS) para reverter so esses elementos especificos para um tom solido legivel.
  // IMPORTANTE: .legenda-glass tambem e reaproveitada em #guia-gastronomico como card branco
  // normal (com override inline background:white) — o seletor abaixo e escopado para
  // .cidade-clean .legenda-glass para nao quebrar esse outro uso.
  await page.addStyleTag({
    content: `
      .cidade-clean .legenda-glass {
        background: #16324f !important;
        border-color: rgba(255,255,255,0.25) !important;
        color: white !important;
      }
      .btn-menu, .btn-mapa-cidade {
        background: white !important;
  ...
  // Heros de cidade (.cidade-clean, paginas full-bleed): escurece a foto de verdade (gradiente
  // aplicado nos pixels via canvas), grava o resultado como PNG em disco e referencia via URL
  // http normal — nao data URI (data URI grande e corrompido/reposicionado pelo Ghostscript
  // apos -dColorConversionStrategy=/sRGB). Arquivo em disco + URL http = caminho comprovado
  // seguro (igual ao #boas-vindas) PARA PAGINAS FULL-BLEED.
  //
  // #pais-hero-bg e diferente: e uma caixa pequena (280px, nao full-page) com border-radius.
  // Testado: nem canvas-recompactado nem border-radius sozinhos resolvem; só funciona com
  // URL direta do arquivo original (sem recompactar) + border-radius:0 (CSS acima). Por isso
  // NAO passa pelo canvas de escurecimento — fica de fora do loop e é tratado à parte, simples.
  const heroData = await page.evaluate(async () => {
    const results = [];
    const imgs = [...document.querySelectorAll('img.cidade-hero-photo')];
    for (const heroImg of imgs) {
      const section = heroImg.closest('.cidade-clean');
      if (!section) continue;
      const dataUrl = await new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          ctx.drawImage(img, 0, 0, W, H);
          const grad = ctx.createLinearGradient(0, 0, 0, H);
          grad.addColorStop(0,    'rgba(10,22,40,0.18)');
          grad.addColorStop(0.28, 'rgba(10,22,40,0.26)');
          grad.addColorStop(0.62, 'rgba(10,22,40,0.52)');
          grad.addColorStop(1.0,  'rgba(10,22,40,0.78)');
          ctx.fillStyle = grad;
          ctx.fillRect(0, 0, W, H);
      ...
      results.push({ id: section.id, dataUrl });
  ...
  const heroTmpFiles = {};
  for (const { id, dataUrl } of heroData) {
    if (!dataUrl) continue;
    const base64 = dataUrl.replace(/^data:image\/png;base64,/, '');
    const tmpName = `_hero_${id}.png`;
    fs.writeFileSync(path.join(baseDir, tmpName), Buffer.from(base64, 'base64'));
    heroTmpFiles[id] = tmpName;
  }

  // IMPORTANTE: background-image apontando pra URL http precisa de um fetch de rede de verdade
  // (diferente de data URI, que ja fica em memoria). Sem esperar esse load, o page.pdf() roda
  // antes da imagem chegar e o PDF sai com a foto ausente/cortada. Por isso cada URL nova e
  // pre-carregada com new Image() + onload antes de seguir para a geracao do PDF.
  await page.evaluate(async (heroTmpFiles, port) => {
    ...
    const urls = Object.keys(heroTmpFiles).map((id) => `http://127.0.0.1:${port}/${heroTmpFiles[id]}`);
    await Promise.all(urls.map(preload));

    Object.keys(heroTmpFiles).forEach((id) => {
      const section = document.getElementById(id);
      if (!section) return;
      section.style.setProperty('background-image', `url(http://127.0.0.1:${port}/${heroTmpFiles[id]})`, 'important');
      section.style.setProperty('background-size', 'cover', 'important');
      section.style.setProperty('background-position', 'center', 'important');
    });

    // #pais-hero-bg: URL direta do arquivo original, sem recompactar (ver nota acima).
    const paisHero = document.getElementById('pais-hero-bg');
    const paisHeroImg = paisHero && paisHero.querySelector('img.cidade-hero-photo');
    if (paisHero && paisHeroImg) {
      paisHero.style.setProperty('background-image', `url(${paisHeroImg.src})`, 'important');
      paisHero.style.setProperty('background-size', 'cover', 'important');
      paisHero.style.setProperty('background-position', 'center', 'important');
      paisHeroImg.style.display = 'none';
    }
  }, heroTmpFiles, port);

  // Espera a rede ficar ociosa (garante que os background-image novos terminaram de carregar
  // e o Chrome ja tem os bytes prontos pra embutir no PDF).
  ...
  // Limpa os PNGs temporarios dos heros de cidade gerados em disco
  for (const tmpName of Object.values(heroTmpFiles)) {
    const p = path.join(baseDir, tmpName);
